...
export function knowledgeGuide(item) {
  ...
  const generated = item.source === 'catalog' && !base ? {
    description: `Conhecimento profissional relacionado a ${item.name}, que pode ser aplicado, documentado e compartilhado de acordo com o contexto da organização.`,
    examples: [`Registrar um caso prático em que ${item.name} foi utilizado, com contexto, decisão tomada e aprendizado para a equipe.`]
  } : null;
  return {
    description: item.description || base?.description || generated?.description || '',
    examples: item.examples ? String(item.examples).split('\n').map(s=>s.trim()).filter(Boolean) : base?.examples || generated?.examples || [],
