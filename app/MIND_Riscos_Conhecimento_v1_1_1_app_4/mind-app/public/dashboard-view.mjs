import {riskLabel, renderRiskTable, renderRiskMethod} from './knowledge-risk-view.mjs';
...
function renderDomainLegend() {
  return `
    <details class="dashboard-method">
      <summary>Como interpretar esta tabela</summary>

      <p>
        Os números representam quantidades de conhecimentos.
        Cada linha reúne os temas da empresa associados a uma
        área do catálogo.
      </p>
      ...
      <ul>
        <li>
          ...
          categoria do catálogo, como Operações e processos ou
          Dados e inteligência artificial. Pode reunir conhecimentos
          de colaboradores de diferentes setores.
        </li>

        <li>
          <strong>Observados:</strong>
          total de conhecimentos desse domínio com declarações nos
          perfis ou publicações vinculadas, incluindo os registros
          históricos considerados na análise. Itens exclusivos do
          catálogo ficam fora.
        </li>

        <li>
          <strong>Risco alto:</strong>
          apenas uma pessoa ativa declara o conhecimento e não há
          publicações vinculadas; ou existem declarações históricas,
          mas nenhum detentor ativo identificado.
        </li>

        <li>
          <strong>Atenção:</strong>
          concentração em uma pessoa ou em um autor, ausência de
          publicações ou último registro há mais de 180 dias,
          quando a avaliação não entrou em Risco alto ou
          Dados insuficientes.
        </li>

        <li>
          <strong>Sem alerta:</strong>
          pelo menos duas declarações ativas, publicações de pelo
          menos dois autores e registro nos últimos 180 dias.
          As regras atuais não apontam alerta, mas isso não garante
          risco zero ou documentação completa.
        </li>

        <li>
          <strong>Sem dados:</strong>
          significa Dados insuficientes. Há registros, mas faltam
          declarações que identifiquem detentores ou datas válidas
          para concluir a avaliação. Não significa ausência de
      ...
      <p>
        Em cada linha:
        <strong>
          Risco alto + Atenção + Sem alerta + Sem dados = Observados.
        </strong>
      </p>

      <p>
        Um mesmo conhecimento pode aparecer em mais de um domínio.
        Por isso, somar as linhas pode ultrapassar o total de
        conhecimentos únicos dos cartões.
      </p>

      <p>
        Detentores são pessoas ativas que declararam o conhecimento.
        Publicações são evidências de registro, não de domínio
        ou documentação completa.
      </p>

      <p>
        A tabela considera a situação atual e todo o histórico
        do setor selecionado, independente do período de
        atividade acima.
      </p>
    </details>
  `;
...
export function renderDashboard(data, areas) {
  ...
  const metrics = [
    ...
    ['Conhecimentos da empresa', s.observedKnowledge || 0, 'Com declarações ou publicações, incluindo o histórico registrado', 'purple'],
    ['Risco alto observado', s.criticalKnowledge || 0, `${s.attentionKnowledge || 0} conhecimentos em atenção · situação atual, sem filtro de período`, 'amber']
  ];
  ...
  return `<div class="row between pagehead"><div><span class="eyebrow">ADMINISTRAÇÃO · MIND</span><h1>Gestão do conhecimento</h1><p class="muted">Entenda o que a empresa compartilha e onde o conhecimento pode circular mais.</p></div><div class="row">${btn('Atualizar', 'dashboard-refresh')}${btn('Exportar CSV', 'dashboard-export', '', 'primary')}</div></div>
    ...
    ${ki ? `<section class="dashboard-section">${title('Riscos do Conhecimento da Empresa','Somente conhecimentos vinculados a pessoas ou publicações. Situação atual e todo o histórico do setor selecionado, independente do período acima. A MIND não define o que a empresa deveria saber.')}
      <div class="knowledge-health-grid">
        <article class="card knowledge-health critical"><span>RISCO ALTO</span><strong>${ki.summary.critical}</strong><p>Sem detentor ativo identificado no histórico, ou uma única declaração ativa sem publicações.</p></article>
        <article class="card knowledge-health attention"><span>ATENÇÃO</span><strong>${ki.summary.attention}</strong><p>Sinais observáveis de concentração, ausência de publicações ou registros antigos.</p></article>
        <article class="card knowledge-health healthy"><span>SEM ALERTA OBSERVADO</span><strong>${ki.summary.healthy}</strong><p>Os critérios atuais não apontam alerta. Isso não significa risco zero.</p></article>
        <article class="card knowledge-health unknown"><span>DADOS INSUFICIENTES</span><strong>${ki.summary.unknown}</strong><p>Há registros na empresa, mas falta identificar detentores ou a data dos registros.</p></article>
      </div>
      <div class="dashboard-grid knowledge-intelligence-grid">
        <section class="card">
  ${title(
    'Registros por domínio',
    'Agrupamento dos conhecimentos observados pelas áreas do catálogo. Não define quais domínios são necessários à empresa.'
  )}

  ${
    ki.domainHealth.some(d => d.concepts)
      ? table(
          [
            'Domínio',
            'Observados',
      ...
      : empty('Nenhum domínio com evidências registradas.')
  }
  ...
  ${renderDomainLegend()}
      ...
      <section class="card dashboard-section">${title('Evidências e motivos dos alertas','Detentores são pessoas ativas que declararam o conhecimento. Publicações são evidências de registro, não de domínio ou documentação completa.')}${renderRiskTable(ki.concepts)}${renderRiskMethod()}</section>
    </section>` : ''}
...
export function dashboardCSV(data, areas) {
  ...
  const rows = [
    ...
    ...(data.intelligence ? [['RISCOS DO CONHECIMENTO DA EMPRESA'],['Escopo','Situação atual e todo o histórico do setor selecionado; independente do período'],['Conhecimentos observados',data.intelligence.summary.concepts],['Itens do catálogo',data.intelligence.summary.catalogItems],['Risco alto',data.intelligence.summary.critical],['Atenção',data.intelligence.summary.attention],['Sem alerta observado',data.intelligence.summary.healthy],['Dados insuficientes',data.intelligence.summary.unknown],[],['EVIDÊNCIAS E ALERTAS'],['Conhecimento','Situação','Detentores declarados ativos','Publicações vinculadas','Autores','Último registro','Motivo principal','Outros sinais'],...data.intelligence.concepts.map(c=>[c.name,riskLabel(c.riskStatus),c.holders.length,c.publications,c.authors,c.lastUsed||'',c.mainReason,c.reasons.slice(1).join(' | ')]),[]] : []),
