...
const tables=['company','areas','positions','users','knowledge_areas','knowledge_items','knowledge_details','user_knowledge','company_guide','posts','post_knowledge','comments','reactions','notifications','audit'];
try{
 ...
 const sourceTables=new Set(source.prepare("SELECT name FROM sqlite_master WHERE type='table'").all().map(r=>r.name));
 for(const table of tables){
  if(!sourceTables.has(table)){console.log(`${table}: não existe no banco antigo; ignorada`);continue;}
