...
import {riskLabel,renderRiskEvidence} from '../public/knowledge-risk-view.mjs';
...
function views(){
 ...
 const sandbox={console,relevance,matchesQuery,knowledgeGuide,renderDashboard,dashboardCSV,riskLabel,renderRiskEvidence,
 ...
 source+=`\nglobalThis.testViews={set(data,person){state=data;user=person;},setView(value){view=value;},setProfile(id){profileId=id;},setKnowledge(id){knowledgeId=id;},setNetwork(value,node=''){knowledgeNetwork=value;selectedNetworkNode=node;},setSearch(value){globalQuery=value;globalType='all';},render,profile,knowledgePage,knowledgeNetworkPage,networkSidePanel,searchResults,searchPage,guidePage,html:()=>app.innerHTML};`;
...
test('rede e detalhe usam situações qualitativas e distinguem autor de detentor',()=>{
 const {v,data,person}=views();
 const raw={...data,people:[person],orgAreas:data.areas,concepts:[{id:'concept-sql',name:'SQL'},{id:'concept-data',name:'Análise de dados'}],itemConcept:[{item_id:'sql',concept_id:'concept-sql'},{item_id:'data',concept_id:'concept-data'}],areaAffinities:[],professionalProfiles:[],profileAffinities:[],capabilities:[],capabilityAffinities:[]};
 const network=buildKnowledgeIntelligence(raw,{clock:new Date('2026-09-26T12:00:00Z')});v.setNetwork(network);
 const html=v.knowledgeNetworkPage();assert.match(html,/Risco alto/);assert.match(html,/Dados insuficientes/);assert.match(html,/publicação vinculada/);assert.match(html,/declaração no perfil/);
 assert.ok(!/cobertura \d|\d+%.*de cobertura|null%|undefined/.test(html));
 v.setNetwork(network,'k:concept-data');assert.match(v.networkSidePanel(),/publicou; domínio não declarado/);
 v.setKnowledge('data');assert.match(v.knowledgePage(),/não há domínio declarado/);assert.ok(!v.knowledgePage().includes('0%'));
 v.setNetwork({...network,concepts:[]});assert.match(v.knowledgePage(),/não gera alerta/);
});

test('rede preserva nós históricos sem nenhuma conexão ativa',()=>{
 const {v,data,person}=views();const raw={...data,people:[{...person,status:'blocked'}],orgAreas:data.areas,concepts:[{id:'historic',name:'SQL'}],itemConcept:[{item_id:'sql',concept_id:'historic'}],areaAffinities:[],professionalProfiles:[],profileAffinities:[],capabilities:[],capabilityAffinities:[]};
 const network=buildKnowledgeIntelligence(raw);v.setNetwork(network,'k:historic');
 assert.match(v.knowledgeNetworkPage(),/data-id="k:historic"/);assert.match(v.networkSidePanel(),/registros históricos foram preservados/);
});
