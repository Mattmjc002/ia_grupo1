...
async function api(req,res,path,b){
 ...
 if(path==='/api/posts'&&req.method==='POST'){
  ...
  if(b.id){const p=(await tx.all('SELECT * FROM posts WHERE id=?',[b.id]))[0];if(!p||p.user_id!==user.id||!canRead(user,p))fail('Você não pode editar essa publicação.',403);await tx.all('UPDATE posts SET title=?,body=?,category=?,area_id=?,audience_level=?,updated_at=? WHERE id=?',[text(b.title,'Título',160),text(b.body,'Texto',10000),text(b.category,'Categoria',80),area,audience,now(),b.id]);}
  else await tx.all('INSERT INTO posts(id,user_id,area_id,audience_level,title,body,category,created_at) VALUES(?,?,?,?,?,?,?,?)',[id(),user.id,area,audience,text(b.title,'Título',160),text(b.body,'Texto',10000),text(b.category,'Categoria',80),now()]);return{ok:true};
 }
