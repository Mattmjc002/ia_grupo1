...
export async function knowledgeIntelligenceData(tx) {
  return {
    people: await tx.all("SELECT u.id,u.name,u.status,u.is_admin,u.position_id,p.area_id,p.name AS position_name,a.name AS official_area_name FROM users u LEFT JOIN positions p ON p.id=u.position_id LEFT JOIN areas a ON a.id=p.area_id WHERE u.is_admin=0"),
    ...
    posts: await tx.all("SELECT p.* FROM posts p JOIN users u ON u.id=p.user_id WHERE u.status='active'"),
...
function statusFromCoverage(coverage, config, known = true) {
  if (!known) return 'unknown';
  if (coverage < config.critical_coverage) return 'critical';
  if (coverage < config.attention_coverage) return 'attention';
  return 'healthy';
}
function riskStatus(score, known = true) {if(!known)return 'unknown';return score>=67?'critical':score>=40?'attention':'healthy';}
...
export function buildKnowledgeIntelligence(data, {excludeUserId = '', clock = new Date(), conceptLimit = 0} = {}) {
  const activePeople = data.people.filter(p=>p.status==='active' && p.id!==excludeUserId);
  ...
  for (const row of data.userKnowledge) {
    ...
    declared.set(key,{proficiency:Number(row.proficiency||3),updated_at:row.updated_at||null,knowledge_id:row.knowledge_id});
  }
  const postById = new Map(data.posts.filter(p=>p.user_id!==excludeUserId).map(p=>[p.id,p]));
  for (const link of data.postKnowledge) {
    const post=postById.get(link.post_id); if(!post||!peopleById.has(post.user_id))continue;
  ...
  for (const person of activePeople) {
    ...
    for (const cid of conceptIds) {
      ...
      const latest = linked.map(p=>p.updated_at||p.created_at).sort().at(-1)||decl?.updated_at||null;
  ...
  const personRelevanceVectors=new Map();
  for(const p of personRows){
    const org=orgAreaProfiles.get(p.area_id),domain=new Map(),prof=new Map();
    const personalWeight=p.confidence>=35?.65:p.confidence>=15?.4:0;
    const orgWeight=1-personalWeight;
    for(const d of p.domains)addScore(domain,d.id,d.share*personalWeight);
    for(const d of org?.domains||[])addScore(domain,d.id,d.share*orgWeight);
    for(const x of p.profiles)addScore(prof,x.id,x.share*personalWeight);
    for(const x of org?.profiles||[])addScore(prof,x.id,x.share*orgWeight);
    personRelevanceVectors.set(p.id,{domain:normalizedShares(domain),profiles:normalizedShares(prof)});
  }

  const holderMap=new Map();
  for(const p of personRows)for(const ev of p.concepts){if(!holderMap.has(ev.concept_id))holderMap.set(ev.concept_id,[]);holderMap.get(ev.concept_id).push({person:p,evidence:ev});}
  const config=data.riskConfig||{critical_coverage:35,attention_coverage:65,relevance_floor:25};
  const conceptRows=[];
  for(const concept of data.concepts){
    const affinities=areaAffinity.get(concept.id)||[],pAff=profileAffinity.get(concept.id)||[],holders=holderMap.get(concept.id)||[];
    let denominator=0,numerator=0,relevantCount=0;
    const relevantPeople=[];
    for(const person of personRows){
      const vec=personRelevanceVectors.get(person.id),domainScore=(vec.domain||[]).reduce((s,d)=>s+d.share*(affinities.find(a=>a.area_id===d.id)?.weight||0)/100,0);
      const profileScore=(vec.profiles||[]).reduce((s,p)=>s+p.share*(pAff.find(a=>a.profile_id===p.id)?.weight||0)/100,0);
      const relevance=clamp(profileScore?domainScore*.7+profileScore*.3:domainScore);
      if(relevance>=config.relevance_floor){relevantCount++;denominator+=relevance;const has=holders.some(h=>h.person.id===person.id);if(has)numerator+=relevance;relevantPeople.push({id:person.id,name:person.name,relevance,has});}
    }
    const known=denominator>0;
    const coverage=known?clamp(numerator/denominator*100):0;
    const posts=postsByConcept.get(concept.id)||[],authors=unique(posts.map(p=>p.user_id)),lastUsed=posts.map(p=>p.updated_at||p.created_at).sort().at(-1)||null;
    const documentationScore=clamp(Math.min(100,posts.length*20+authors.length*12));
    const concentrationRisk=holders.length===0?100:holders.length===1?95:holders.length===2?68:holders.length===3?45:holders.length===4?28:15;
    const coverageRisk=known?100-coverage:100;
    const docRisk=100-documentationScore, freshRisk=ageRisk(lastUsed,clock);
    const importance=known?clamp(denominator/Math.max(1,relevantCount)):0;
    const raw=.55*coverageRisk+.2*concentrationRisk+.15*docRisk+.1*freshRisk;
    const risk=known?clamp(raw*(.6+.4*importance/100)):0;
    ...
    conceptRows.push({id:concept.id,name:concept.name,source:concept.source,knowledge_id:itemMappings[0]?.id||'',primaryAreas,
      ...
      holders:holders.map(h=>({id:h.person.id,name:h.person.name,confidence:h.evidence.confidence,proficiency:h.evidence.proficiency,publications:h.evidence.publications})),
      relevantPeople:relevantPeople.sort((a,b)=>b.relevance-a.relevance),relevantCount,coverage,coverageStatus:statusFromCoverage(coverage,config,known),risk,riskStatus:riskStatus(risk,known),
      documentation:documentationScore,publications:posts.length,authors:authors.length,lastUsed,importance,known});
  }

  conceptRows.sort((a,b)=>b.risk-a.risk||a.coverage-b.coverage||a.name.localeCompare(b.name,'pt-BR'));
  ...
  const domainHealth=data.knowledgeAreas.map(area=>{
    const related=conceptRows.filter(c=>c.areaAffinities.some(a=>a.id===area.id&&a.weight>=70)&&c.known);
    const coverage=related.length?clamp(related.reduce((s,c)=>s+c.coverage,0)/related.length):0;
    return {id:area.id,name:area.name,concepts:related.length,coverage,critical:related.filter(c=>c.riskStatus==='critical').length,attention:related.filter(c=>c.riskStatus==='attention').length,healthy:related.filter(c=>c.riskStatus==='healthy').length};
...
export function simulateKnowledgeExit(data, userId, clock = new Date()) {
  ...
  const impacts=baseline.concepts.filter(c=>held.has(c.id)).map(before=>{
    const next=afterMap.get(before.id)||{coverage:0,risk:100,riskStatus:'critical',holders:[],relevantPeople:[]};
    const candidates=(next.relevantPeople||[]).filter(p=>!p.has).slice(0,3);
    return {id:before.id,name:before.name,knowledge_id:before.knowledge_id,beforeCoverage:before.coverage,afterCoverage:next.coverage,coverageDrop:Math.max(0,before.coverage-next.coverage),beforeRisk:before.risk,afterRisk:next.risk,riskIncrease:Math.max(0,next.risk-before.risk),beforeStatus:before.riskStatus,afterStatus:next.riskStatus,remainingHolders:next.holders.length,recommendedRecipients:candidates};
  }).sort((a,b)=>b.riskIncrease-a.riskIncrease||b.coverageDrop-a.coverageDrop||a.name.localeCompare(b.name,'pt-BR'));
  return {person:{id:person.id,name:person.name,officialArea:person.official_area_name||'',domains:person.domains.slice(0,5),profiles:person.profiles.slice(0,5)},summary:{knowledge:held.size,criticalAfter:impacts.filter(i=>i.afterStatus==='critical').length,newCritical:impacts.filter(i=>i.beforeStatus!=='critical'&&i.afterStatus==='critical').length,orphaned:impacts.filter(i=>i.remainingHolders===0).length,averageCoverageDrop:impacts.length?clamp(impacts.reduce((s,i)=>s+i.coverageDrop,0)/impacts.length):0},impacts};
}
